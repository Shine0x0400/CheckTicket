...
import com.zjl.checkticket.SharedPreferenceUtil;
...
public class SettingsActivity extends AppCompatPreferenceActivity {
    ...
    public static class DataSyncPreferenceFragment extends PreferenceFragment implements Observer {
        ...
        public void onCreate(Bundle savedInstanceState) {
            ...
            final String name = SharedPreferenceUtil.getInstance().getParkName();
            if (name != null) {
                parkPref.setSummary(name);
            }
                ...
                if (type.equals(TicketDataManager.MessageType.PARKS_DATA_CHANGED)) {
                    ...
                    if (parks != null && !parks.isEmpty()) {
                        ...
                        synchronized (TicketDataManager.SYNCHRONIZE_LOCK_PARKS) {
                            int idx = 0;
                            for (Park park : parks) {
                                if (!TextUtils.isEmpty(park.getId())) {
                                    entries[idx] = park.getName();
                                    values[idx] = SharedPreferenceUtil.getInstance()
                                            .makeParkSavedString(park.getId(), park.getName());
                                }
                                idx++;
                            }
                        }
